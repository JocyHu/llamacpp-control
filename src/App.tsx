                ...
                <div className="bg-card p-6 rounded-[2.5rem] border border-border/60 shadow-xl space-y-6 transition-all text-foreground">
                  ...
                  <div className="space-y-5 text-foreground">
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-2">
                      <div className="space-y-1"><label className="text-[10px] font-bold text-muted-foreground ml-1 uppercase tracking-tighter text-[9px]">{t("kv_cache_k")}</label><select value={activeProfile.cache_type_k} onChange={(e) => updateProfileField(activeProfile.id, "cache_type_k", e.target.value)} className="w-full bg-secondary/40 border border-border/60 rounded-xl px-3 py-2 text-xs outline-none transition-all focus:ring-2 focus:ring-primary/20 text-foreground hover:bg-secondary/60 cursor-pointer">{CACHE_TYPES.map(t => <option key={t} value={t} className="bg-popover text-popover-foreground">{t}</option>)}</select></div>
                      <div className="space-y-1"><label className="text-[10px] font-bold text-muted-foreground ml-1 uppercase tracking-tighter text-[9px]">{t("kv_cache_v")}</label><select value={activeProfile.cache_type_v} onChange={(e) => updateProfileField(activeProfile.id, "cache_type_v", e.target.value)} className="w-full bg-secondary/40 border border-border/60 rounded-xl px-3 py-2 text-xs outline-none transition-all focus:ring-2 focus:ring-primary/20 text-foreground hover:bg-secondary/60 cursor-pointer">{CACHE_TYPES.map(t => <option key={t} value={t} className="bg-popover text-popover-foreground">{t}</option>)}</select></div>
                    </div>
                  </div>
                </div>
                      ...
                      {[
                        ...
                        <div key={p.f} className={cn("space-y-1.5 transition-opacity", !(activeProfile as any)[p.u] && "opacity-40")}>
                          ...
                          <input 
                            ...
                            onChange={(e) => updateProfileField(activeProfile.id, p.f as any, ['top_k', 'batch_size', 'ubatch_size'].includes(p.f) ? parseInt(e.target.value) : parseFloat(e.target.value))}
